...
export class ContaReceberFormComponent implements OnInit {
  ...
  categoriaOptions = [
    { label: 'Venda a Prazo', value: 'VENDA_PRAZO' },
    { label: 'Serviço Prestado', value: 'SERVICO' },
  ...
  carregarConta(id: number) {
    ...
    this.contaReceberService.buscarPorIdAPI(id).subscribe({
      next: conta => {
  ...
  salvar() {
    ...
    const obs = this.isEditMode && this.contaId
      ? this.contaReceberService.atualizarAPI(this.contaId, payload)
      : this.contaReceberService.criarAPI(payload);
    ...
    obs.subscribe({
      next: () => {
        ...
        setTimeout(() => this.router.navigate(['/financeiro/receber']), 1200);
      },
      error: err => {
        this.salvando = false;
